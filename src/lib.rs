...
fn main() {
    ...
    let pjuri = PjUri::from(url.as_str()).expect("Failed to create PjUri from URL");
    ...
    let pjuri_parsed = PjUri::from(&pjuri_str).amount.label.message.expect("Failed to parse PjUri from string");
