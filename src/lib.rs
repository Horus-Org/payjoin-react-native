...
fn main() {
    ...
    let pjuri = PjUri::from_str(url.as_str()).expect("Failed to create PjUri from URL");
    ...
    let pjuri_parsed = PjUri::from(&pjuri_str).expect("Failed to parse PjUri from string");
